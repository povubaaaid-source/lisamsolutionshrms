# Lisam Solutions HRMS System Documentation and Implementation Blueprint

Project: Lisam Solutions HRMS Next.js Application

Analysis date: 2026-05-19

Primary scope: the HRMS web application, its business modules, frontend architecture, API contract, backend setup plan, database blueprint, risks, and roadmap.
...
| Backend inside Next.js app | Production backend route handlers are planned. Current development mode uses an Axios mock adapter backed by browser storage. |
...
| Prepare for backend migration | Keep frontend API calls normalized under `/v1/*` so a production backend API can replace the development mock API. |
...
Browser
  -> Next.js App Router
  -> Global Providers: Auth, Toasts, Top Loader
  -> Route Guard: token, role, permissions
  -> Shared Layout: Sidebar, Navbar, Dashboard Shell
  -> Feature Modules: HR, Payroll, Leads, Projects, Finance, Recruitment
  -> Axios API Client
  -> API Contract Normalizer (/v1 routes)
  -> Development Mock API or Production Backend API
  -> Database, queues, file storage, notifications, and integrations
```
...
- Use the payroll resource contract in this document as the backend implementation model.
...
- Finance document actions can show "queued for backend API wiring", meaning backend work is incomplete.
...
```text
API Relationship Map
Company -> Admins, Employees, Clients, Projects
Employee -> Attendance, Leaves, Payroll, Tasks, Time Logs
Department/Designation/Shift -> Employee Profile
Client -> Projects, Invoices, Payments, Tickets
Project -> Tasks, Files, Time Logs, Expenses, Invoices
Lead -> Source, Status, Category, Followups, Proposals, Client Conversion
Payroll -> Salary Setup, Attendance, Leaves, Expenses, Payslips
Ticket -> Replies, Files, Notifications
User -> Permissions, Activities, Notifications
```
...
| Mock persistence | `localStorage` key `browser_mock_api_store` |
...
Backend server setup should expose the `/api/v1` contract documented here and run database migrations, seeders, queue workers, and scheduled jobs before connecting the frontend in live mode.
...
If building a clean production backend, match the `/v1/*` contract in this document and the API contract documentation.
...
The production database plan below maps the HRMS frontend contract to the database tables required for a complete backend implementation.
...
users 1..N attendance, leaves, payroll, notifications
employees N..1 departments, designations, shift_types
clients 1..N projects, invoices, payments, tickets
projects 1..N tasks, time_logs, invoices, expenses, files
leads N..1 lead_sources, lead_statuses, lead_categories
leads 1..N followups, files, activities, proposals
payroll uses employee salary, salary groups, salary components, attendance, leaves, holidays, expenses, and tax slabs
```
...
1. Finalize backend API contract from this document and the API contract documentation.
...
- Backend runtime supported by the implementation team.
- SQL database, preferably MySQL/PostgreSQL.
- Redis for queues, cache, and session storage where required.
- Queue worker.
- Scheduler/cron.
