    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between bg-white px-6 py-4 shadow-sm -mx-6 -mt-6 mb-6 border-b border-gray-100">
          <div>
            <h1 className="text-base font-semibold text-gray-700 uppercase tracking-widest font-black">Create Invoice</h1>
            <div className="text-xs text-gray-500 flex items-center space-x-1 mt-1">
              ...
              <span>/</span>
              <Link href="/invoices" className="hover:text-primary transition-colors font-bold uppercase tracking-tighter">Invoices</Link>
              <span>/</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto">
          ...
          <Card className="p-6 shadow-sm border-gray-100 relative">
            ...
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-5">Invoice Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Invoice # <span className="text-red-500">*</span></label>
                <div className="relative">
                  <FileText className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                  <input name="invoice_number" value={formData.invoice_number} onChange={handleChange}
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Issue Date <span className="text-red-500">*</span></label>
                <div className="relative">
                  <CalendarIcon className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                  <input name="issue_date" value={formData.issue_date} onChange={handleChange} type="date"
                    className="w-full border-gray-200 rounded p-2.5 pl-9 text-xs font-bold focus:ring-1 focus:ring-primary/20 outline-none" required />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <CalendarIcon className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                  <input name="due_date" value={formData.due_date} onChange={handleChange} type="date"
                    className="w-full border-gray-200 rounded p-2.5 pl-9 text-xs font-bold focus:ring-1 focus:ring-primary/20 outline-none" required />
                </div>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-3">
              ...
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  <input value={item.quantity} onChange={e => handleItemChange(i, "quantity", e.target.value)}
                    type="number" min="1"
                    className="col-span-2 border-gray-200 rounded p-2 text-xs font-bold focus:ring-1 focus:ring-primary/20 outline-none" />
                  <input value={item.unit_price} onChange={e => handleItemChange(i, "unit_price", e.target.value)}
