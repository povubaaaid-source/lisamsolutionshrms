...
export default function CreateTaskCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (localStorage.getItem("token") === "mock_token_12345") {
        setTimeout(() => { router.push("/task-category"); router.refresh(); }, 800);
        return;
      }
      await api.post("/task-category", { category_name: categoryName });
      router.push("/task-category");
      router.refresh();
    } catch (err: any) {
      setError(err.response?.data?.message || err.response?.data?.error || "Failed to create task category.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between bg-white px-6 py-4 shadow-sm -mx-6 -mt-6 mb-6 border-b border-gray-100">
          <div>
            <h1 className="text-base font-semibold text-gray-700 uppercase tracking-widest font-black">Add Task Category</h1>
            <div className="text-xs text-gray-500 flex items-center space-x-1 mt-1">
              ...
              <span>/</span>
              <Link href="/task-category" className="hover:text-primary transition-colors font-bold uppercase tracking-tighter">Task Categories</Link>
              <span>/</span>
              <span className="text-gray-700 font-bold uppercase tracking-tighter">Add</span>
            </div>
          </div>
          <Link href="/task-category">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input value={categoryName} onChange={e => setCategoryName(e.target.value)}
                  type="text" placeholder="e.g. Bug Fix"
