...
import { Plus, Sliders, Calendar, Search, MoreVertical, RefreshCw, Eye, Edit, Trash2 } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

const initialInvoices = [
  { id: 1, client: "Acme Corp", total: "$1,500.00", frequency: "Monthly", nextInvoice: "2026-06-01", status: "Active" },
  { id: 2, client: "Globex Corp", total: "$2,200.00", frequency: "Weekly", nextInvoice: "2026-05-15", status: "Inactive" },
  { id: 3, client: "Initech", total: "$800.00", frequency: "Monthly", nextInvoice: "2026-06-10", status: "Active" },
];
          ...
          <Card className="p-6 bg-gray-50/50 border border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="flex items-end space-x-2">
                <Button className="bg-primary text-white text-[10px] h-9 px-4 flex-1">Apply</Button>
                <Button className="bg-gray-200 text-gray-600 text-[10px] h-9 px-4">Reset</Button>
              </div>
            </div>
          </Card>
        )}
        ...
        <Card className="p-0 overflow-hidden shadow-sm border border-gray-100">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-white">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select className="text-[10px] font-bold border-gray-200 rounded p-1 bg-white focus:outline-none focus:ring-1 focus:ring-primary/20">
                <option>10</option>
                <option>25</option>
                <option>50</option>
              </select>
            ...
            <tbody className="divide-y divide-gray-100">
              {invoices.map((invoice, i) => (
                <tr key={invoice.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-xs text-gray-500 font-medium">{invoice.nextInvoice}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                      invoice.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                    }`}>
                      {invoice.status}
                    </span>
                  </td>
