...
import { ArrowLeft, Save, RefreshCw, LayoutPanelLeft } from "lucide-react";
...
export default function CreateNoticePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.error || "Failed to create notice.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between bg-white px-6 py-4 shadow-sm -mx-6 -mt-6 mb-6 border-b border-gray-100">
          <div>
            <h1 className="text-base font-semibold text-gray-700 uppercase tracking-widest font-black">Add Notice</h1>
            <div className="text-xs text-gray-500 flex items-center space-x-1 mt-1">
              ...
              <Link href="/notices" className="hover:text-primary transition-colors font-bold uppercase tracking-tighter">Notice Board</Link>
              <span className="font-bold">/</span>
              <span className="text-gray-700 font-bold uppercase tracking-tighter">Add Notice</span>
            </div>
          </div>
          <Link href="/notices">
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Notice Heading <span className="text-red-500">*</span></label>
                <div className="relative">
                  <LayoutPanelLeft className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                  <input 
                    name="heading"
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">To <span className="text-red-500">*</span></label>
                <select 
                  name="to"
                  value={formData.to}
                  onChange={handleChange}
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  ...
                  onChange={handleChange}
                  className="w-full border-gray-200 rounded p-2.5 text-xs font-bold focus:ring-1 focus:ring-primary/20 outline-none h-40 transition-all"
