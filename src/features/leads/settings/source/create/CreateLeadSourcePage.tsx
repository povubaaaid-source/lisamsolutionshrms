...
import { ArrowLeft, Save, RefreshCw, Globe } from "lucide-react";
import Link from "next/link";
import { useState } from "react";
import { useRouter } from "next/navigation";
import api from "@/lib/api";

export default function CreateLeadSourcePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/lead-source", { type });
      router.push("/lead-settings");
      router.refresh();
    } catch (err: any) {
      setError(err.response?.data?.message || "Failed to create lead source.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between bg-white px-6 py-4 shadow-sm -mx-6 -mt-6 mb-6 border-b border-gray-100">
          <div>
            <h1 className="text-base font-semibold text-gray-700 uppercase tracking-widest font-black">Add Lead Source</h1>
            <div className="text-xs text-gray-500 flex items-center space-x-1 mt-1">
              ...
              <span>/</span>
              <span className="text-gray-700 font-bold uppercase tracking-tighter">Add Source</span>
            </div>
          </div>
        ...
        <Card className="p-8 max-w-2xl mx-auto shadow-sm border-gray-100">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Source Name <span className="text-red-500">*</span></label>
              <div className="relative">
                <Globe className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                <input value={type} onChange={e => setType(e.target.value)}
                  type="text" placeholder="e.g. Website, Referral, Social Media"
