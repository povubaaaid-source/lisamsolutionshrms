  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (localStorage.getItem("token") === "mock_token_12345") {
        setTimeout(() => { router.push("/client-settings"); router.refresh(); }, 800);
        return;
      }
      await api.post("/client-category", { category_name: categoryName });
      router.push("/client-settings");
      router.refresh();
    } catch (err: any) {
      setError(err.response?.data?.message || "Failed to create client category.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between bg-white px-6 py-4 shadow-sm -mx-6 -mt-6 mb-6 border-b border-gray-100">
          <div>
            <h1 className="text-base font-semibold text-gray-700 uppercase tracking-widest font-black">Add Client Category</h1>
            <div className="text-xs text-gray-500 flex items-center space-x-1 mt-1">
              ...
              <span>/</span>
              <Link href="/client-settings" className="hover:text-primary transition-colors font-bold uppercase tracking-tighter">Client Settings</Link>
              <span>/</span>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Users className="absolute left-3 top-2.5 h-3.5 w-3.5 text-gray-400" />
                <input value={categoryName} onChange={e => setCategoryName(e.target.value)}
                  type="text" placeholder="e.g. VIP Client"
